# Socialware 三层架构设计 — L1/L2/L3 Package + Fork 策略
...
| 维度 | A: 纯 Fork 链 | B: 纯分支 | C: Package+Worktree | **D: Package+Fork** |
|------|:---:|:---:|:---:|:---:|
| L1/L2 边界清晰度 | 差 (隐性目录约定) | 差 (分支≠隔离) | 强制 (包边界) | **强制 (包边界)** |
| L1→L2 同步 | merge (可能冲突) | merge (同 repo) | 版本升级 | **版本升级** |
| L2→L1 回馈 | cherry-pick (易出错) | cherry-pick (同 repo) | 独立 PR | **独立 PR** |
| L3 提炼到 L2 | 跨 repo PR | cherry-pick | cherry-pick | **跨 repo PR + review** |
| 租户隔离 | 好 | **无** | **部分 (demo 无隔离)** | **完全隔离** |
| 提炼效率 | 需 fetch (秒级额外开销) | 原生 (零开销) | 混合 | **需 fetch (秒级额外开销)** |
| 新租户成本 | ~30 分钟 | ~5 分钟 | ~5 分钟 (demo) | **~30 分钟 (可脚本化)** |
| Code review | 原生 PR | 无原生流程 | 混合 | **原生 PR** |
| 误操作爆炸半径 | 单个 repo | 所有租户 | 所有 demo 租户 | **单个 repo** |
| 子租户 (L3→L3') | fork 链加深 | 分支爆炸 | 混合问题加倍 | **plugin 或 fork** |
...
方案 D 在 L1/L2 层面与方案 C 相同（L1 为独立包），但 L3 统一走 fork，解决了方案 C 的所有问题：
...
**选定方案 D：Package + Fork（统一 Fork）策略。**

核心理由：
1. L1/L2 之间通过包依赖强制隔离，从结构上消除边界模糊
2. L3 统一走 fork，租户隔离由 GitHub 平台保证，无安全隐患
3. 提炼效率的微小差距（fetch 秒级开销）远小于 PR review 带来的质量收益
4. 新租户成本可通过脚本抹平
5. 管理模型统一，无 demo/交付状态切换成本
...
└────────────────────────────┬────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│  L2: autoservice (独立 repo, depends on socialware)              │
...
### 3.2 L1: socialware 包结构

```
socialware/                              # L1 独立 repo
├── socialware/                          # Python package
...
```
autoservice/                             # L2 独立 repo
├── autoservice/                         # L2 业务包
│   ├── __init__.py
│   ├── customer_manager.py              # 客户管理 (从原 autoservice/ 搬出)
│   ├── crm.py                           # CRM (从原 autoservice/ 搬出)
│   ├── rules.py                         # 行为规则
...
├── .mcp.json                            # MCP 服务配置
├── pyproject.toml                       # depends on socialware
├── CLAUDE.md
└── Makefile
```
...
dependencies = [
    "socialware>=0.1.0",      # L1 框架
    # L2 自身的额外依赖 (如有)
]

# 开发态: 本地 editable 引用 L1 源码
[tool.uv.sources]
socialware = { path = "../socialware", editable = true }
```
...
├── skills/
│   ├── customer-service/                # 继承自 L2
│   └── cinnox-demo/                     # 租户专属
├── .autoservice-info.yaml               # customer: cinnox
└── (其余继承自 L2, 通过 git merge upstream/main 保持同步)
```
...
```
~/work/
├── socialware/                  # L1 (git clone)
├── autoservice/                 # L2 (git clone)
├── autoservice-cinnox/          # L3 (git clone of fork)
└── autoservice-acme/            # L3 (git clone of fork)
```

L2 的 `pyproject.toml` 通过 `[tool.uv.sources]` 指向本地 L1:
...
改 L1 代码 → 保存 → L2 立即生效, 零延迟。

L3 的开发者同样可以用 editable install 引用本地 L2:

```toml
# L3 的 pyproject.toml (开发态)
[tool.uv.sources]
socialware = { path = "../socialware", editable = true }
# 如果 L2 也发布为包:
# autoservice = { path = "../autoservice", editable = true }
```
...
```
开发态:
  L1 本地修改 → 保存 → L2 editable install 立即生效

发布态:
  L1 tag + publish → L2 更新 pyproject.toml 版本号 → uv sync
```
...
# (可选) publish to private PyPI

# 每个 L2 升级
cd ~/work/autoservice/
# 更新 pyproject.toml: socialware>=0.2.0
uv sync
```
...
当前代码中最大的灰色地带是"框架提供机制, 但硬编码了业务策略"的模块。拆分原则:

### 6.1 config.py 拆分

```python
# L1: socialware/config.py — 只有机制
...
# L2: autoservice/domain_config.py — 业务数据
LANG_CONFIGS = {
    'zh': {
        'marketing': { ... },
        'customer-service': { ... },
...
def get_cs_config(domain: str, config_path=None, language='zh') -> dict:
    ...
    return socialware.get_domain_config(domain, config_path, defaults)
```

### 6.2 session.py 拆分

```python
# L1: socialware/session.py — 框架机制
...
# L2: autoservice/domain_session.py — 业务策略
...
def init_cs_session(domain: str, config=None) -> tuple[str, Path]:
    ...
    return socialware.init_session(prefix, base_dir, ...)
```
...
| 问题 | 是 → L1 | 否 → L2 |
|------|---------|---------|
| 换个完全不同的 app, 这个函数还有用吗? | L1 | L2 |
| 删掉这个函数, 框架还能加载插件和接入渠道吗? | L1 | L2 |
| 这个函数的参数名/返回值含有 "customer"/"agent"/"CRM" 等客服词汇? | L2 | (看上面两条) |
| 其他非客服类 app 会需要一个类似的但参数不同的版本吗? | L1 (抽象版) | L2 (具体版) |
...
**为什么全 fork 加剧了这个问题:** worktree 模式下，开发者在同一个 repo 工作，对 main 的更新有天然感知。fork 是独立 repo，不 fetch 就看不到 upstream 有更新。
...
```
# .github/CODEOWNERS
# L2 核心代码 — 修改需要 L2 maintainer review
/autoservice/    @h2oslabs/framework-team
/feishu/         @h2oslabs/framework-team
/web/            @h2oslabs/framework-team
/Makefile        @h2oslabs/framework-team
/pyproject.toml  @h2oslabs/framework-team
      ...
      - name: Check for L2 core modifications
        run: |
          ...
          if [ "$CUSTOMER" != "~" ] && [ -n "$CUSTOMER" ]; then
            CORE_CHANGES=$(git diff --name-only origin/main...HEAD | grep -E '^(autoservice/|feishu/|web/)' || true)
...
| 目录 | 可修改? | 说明 |
|------|---------|------|
| plugins/你的租户/ | YES | 租户专属, 自由修改 |
| skills/你的租户/ | YES | 租户专属, 自由修改 |
| .autoservice-info.yaml | YES | 租户元数据 |
| autoservice/ | NO | L2 核心, 改了下次 merge 会冲突。改进请提 PR 到 upstream |
| feishu/ | NO | L1 渠道层 |
| web/ | NO | L1 渠道层 |
| pyproject.toml | 谨慎 | 可添加依赖, 不要修改已有依赖版本 |
```
...
**问题:** L1 (socialware) 发布了 breaking change（例如 `plugin_loader.discover()` 的参数变了），L2 升级后修改了调用代码，推送到 main。所有 L3 fork 下次 sync 时，如果未同步升级 socialware，运行会报错。
...
```
socialware 0.x.y — 快速迭代期, minor 版本可能有 breaking change
socialware 1.0.0+ — 稳定期:
  - patch (1.0.x): bug fix, 完全兼容
  - minor (1.x.0): 新功能, 向后兼容
  - major (2.0.0): breaking change, 提供迁移指南
```

Breaking change 时:
1. 先发一个 minor 版本标记旧 API 为 deprecated (加 warning)
2. 下一个 major 版本才真正移除
3. L2 在升级 major 版本时, 同步更新所有 import 路径, 并在 CHANGELOG 中写清楚 L3 需要改什么

**(2) L2 的 pyproject.toml 版本约束**

```toml
dependencies = [
    "socialware>=0.2.0,<0.3",   # 锁定 minor 范围, 避免自动升到 breaking version
]
```
...
```bash
# review-forks.sh — 找出各 fork 对 autoservice/ 的改动
FORKS=("cinnox/autoservice" "acme/autoservice")
for fork in "${FORKS[@]}"; do
    echo "=== $fork ==="
    gh api "repos/$fork/compare/h2oslabs:AutoService:main...$fork:main" \
        --jq '.files[] | select(.filename | startswith("autoservice/")) | .filename' \
        2>/dev/null
    echo ""
done
```

如果发现某个 fork 修改了 `autoservice/` 下的文件，主动联系该 fork 的维护者讨论是否应该提 PR。
...
### Phase 1: 创建 L1 repo (socialware)

1. 创建 `socialware` 仓库
2. 从当前 `autoservice/` 中提取 L1 模块
3. 拆分灰色地带 (config, session, permission, api_interfaces)
4. 将 `feishu/`, `web/` 移入 `socialware/channels/`
5. 建立 `pyproject.toml`, 可本地 editable install
6. 验证: `pip install -e .` 后 `from socialware import ...` 可用

### Phase 2: 重构当前 repo 为 L2 (autoservice)

1. `autoservice/` 包只保留 L2 模块 (customer_manager, crm, rules, domain_*)
2. `pyproject.toml` 添加对 `socialware` 的依赖
3. 更新 import 路径: `from autoservice.core import ...` → `from socialware import ...`
4. 验证: `make run-channel` 和 `make run-web` 正常启动

### Phase 3: 将现有租户转为 L3 fork

1. 为 cinnox 创建独立 fork repo
2. 将 `plugins/cinnox/` 和 `skills/cinnox-demo/` 移入 fork
3. L2 main 分支只保留 `plugins/_example/`
4. 配置 fork 的 upstream remote
5. 验证: fork 中 `git merge upstream/main` 无冲突
